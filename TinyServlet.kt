package com.yq.mzl.api.gw.s

import com.line.arch.commons.std.Creator
 ...
 *  // 集成FreeMarker
 *  String tplPath = Main.class.getClassLoader().getResource("templates").getPath();
 *  context.setTemplate("FreeMaker", tplPath, ".ftl");
 *  // 启动服务
 *  server.start();
 *  server.join();
 */
...
class RouteResolver {
    ...
    var pkg: String = ""
    var creator: Creator<Class<*>>? = null
    ...
    constructor(pkg: String, creator: Creator<Class<*>>?) {
        this.pkg = pkg
    ...
    private fun load(pkg: String, creator: Creator<Class<*>>?) {
        val array = Types.getPkgClasses(pkg) {
            !it.isAnnotation && it.getAnnotation(Controller::class.java) != null
        }
        for (c in array) {
            val constructor = c.getDeclaredConstructor()
            if (constructor != null) {
                val instance = (creator ?: Creator.CLASS).create(c)
                val ca = c.getAnnotation(RequestMapping::class.java)
                this.resolveMethods(instance, ca, c!!)
            }
        }
    }
    ...
    internal fun takeMatch(pathInfo: String): Action? {
        return this.actionMap.getOrDefault(pathInfo, null)
    }
}
...
class TinyServletHandler() : AbstractHandler() {
    ...
    constructor(pkgScope: String, creator: Creator<Class<*>>?) : this() {
    ...
    private fun notfound(rsp: HttpServletResponse) {
        rsp.status = 404
    ...
    private fun internalError(err: Throwable, rsp: HttpServletResponse) {
        rsp.status = 500
        rsp.setHeader("Content-Type", "text/html; charset=utf-8")
        rsp.resetBuffer()
        val stream = rsp.outputStream
    ...
    /** 设置模板  */
    fun setTemplate(engine: String?, tpPath: String, suffix: String) {
...
/** Http上下文 */
class HttpContext(private var req: HttpServletRequest, private val rsp: HttpServletResponse) {
    ...
    fun render(name: String, data: Any?): Error? {
        try {
            ...
            t.process(data, this.rsp.writer)
            this.rsp.setHeader("Content-Type", "text/html;charset=UTF-8")
            this.rsp.status = 200
        } catch (e: Throwable) {
            e.printStackTrace()
...
internal object FreeMakerLoader {
    ...
    fun configure(tpPath: String, suffix: String) {
        this.cfg = Configuration()
        this.cfgSuffix = suffix
        try {
            this.cfg!!.setDirectoryForTemplateLoading(File(tpPath))
        } catch (ex: Throwable) {
            ex.printStackTrace()
            System.exit(1)
        }
    ...
    fun getTemplate(name: String): Template {
        return this.cfg!!.getTemplate(name + cfgSuffix)
    }
}
